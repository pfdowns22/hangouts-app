...
- **Sponsored events (self-serve):** sponsors visit **`/sponsor`** (public, no login), submit an event + pick a package, and pay via **Stripe Checkout** (`api/sponsor-checkout.js`). On payment, `api/stripe-webhook.js` (Firebase Admin) writes an active doc to `artifacts/<appId>/public/data/sponsored` with `activeFrom`/`activeTo`; it shows atop matching feeds labeled "Sponsored" with `impressions`/`clicks` tracked. **Register the webhook** `https://<deploy>/api/stripe-webhook` (event `checkout.session.completed`) in Stripe and set `STRIPE_SECRET_KEY`, `STRIPE_WEBHOOK_SECRET`, `FIREBASE_SERVICE_ACCOUNT`. Money lands in your Stripe balance; monitor in the Stripe dashboard. You can still author placements manually via the Firebase Console.
- **Email notifications:** when an event is proposed to a group, members are emailed via Resend (`api/notify-proposal.js`); set `RESEND_API_KEY` + `RESEND_FROM` (verified domain). Chat activity uses in-app unread badges.
